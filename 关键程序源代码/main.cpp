...
int main()
{
	FileManager mg;//managing file io.
	ConsoleBoard consoleBoard;//User interface for standard menu .

	while (consoleBoard.whetherExitDateBase() == false)//Circular running of program.
	{
		consoleBoard.showMenu();

		try
		{
			switch (consoleBoard.getDirective())//get user's direction from console board.
			{
		...
		catch (erroType error)//deal with all serious errors.
		{
			switch (error)
			{
